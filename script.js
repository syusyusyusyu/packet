...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ネットワークシミュレーションクラスに歌詞表示機能を追加
  // LyricsNetworkSimulation クラス内の displayViewerLyric メソッドを拡張する
  if (window.simulation) {
    // オリジナルのメソッドを保存
    const originalDisplayViewerLyric = window.simulation.displayViewerLyric;
    
    // メソッドをオーバーライド
    window.simulation.displayViewerLyric = function(text) {
      // オリジナルの処理を実行
      originalDisplayViewerLyric.call(this, text);
      ...
      if (mobileLyricsContainer) {
        ...
        lyricItem.className = 'mb-1 p-1 rounded bg-space-800 bg-opacity-50';
        lyricItem.textContent = text;
        
        // 最新の歌詞を先頭に表示
        if (mobileLyricsContainer.firstChild) {
          mobileLyricsContainer.insertBefore(lyricItem, mobileLyricsContainer.firstChild);
        } else {
          mobileLyricsContainer.appendChild(lyricItem);
        }
        
        // 古い歌詞を削除（最大10個まで表示）
        while (mobileLyricsContainer.children.length > 10) {
          mobileLyricsContainer.removeChild(mobileLyricsContainer.lastChild);
        }
      }
    };
  }
